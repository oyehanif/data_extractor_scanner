...
import '../widgets/card_details_display.dart';

class CardScannerScreen extends ConsumerWidget {
  const CardScannerScreen({Key? key}) : super(key: key);
        ...
        child: Column(
          children: [
            ...
            _ScanButtons(
              isLoading: state.isLoading,
              onCameraPressed: () =>
                  ref.read(cardScannerProvider.notifier).scanFromCamera(),
              onGalleryPressed: () =>
                  ref.read(cardScannerProvider.notifier).scanFromGallery(),
              onResetPressed: () =>
                  ref.read(cardScannerProvider.notifier).reset(),
            ),
            if (state.cardDetails != null)
              CardDetailsDisplay(cardDetails: state.cardDetails!),
          ],
        ),
      ),
    );
          ...
          if (imageFile != null)
            Image.file(
              ...
              errorBuilder: (context, error, stackTrace) {
                debugPrint('CardPreview: Image load error: $error');
